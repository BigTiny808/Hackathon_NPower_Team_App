...
let streak = 0;
let mistakeCount = 0;
let playerWpmHistory = [];

typingInput.addEventListener("input", () => {
  // START ON FIRST KEYPRESS
  if (!raceActive && typingInput.value.length === 1) {
    startTime = Date.now();
    raceActive = true;

    terminalStatus.textContent = "STATUS: BREACHING";

    // NOW SHOW REAL AI WPM
...
function getAveragePlayerWpm() {
  // Default AI skill for first game
  if (playerWpmHistory.length === 0) return 35;

  const total = playerWpmHistory.reduce((sum, wpm) => sum + wpm, 0);

  return Math.round(total / playerWpmHistory.length);
}

function setComment(type) {
  const now = Date.now();
  if (
    now - lastCommentaryTime < 3000 &&
    !["win", "lose", "start"].includes(type)
  )
    return;
...
function calculateStats() {
  ...
  const accuracy =
    totalTyped > 0 ? Math.round((correctCharacters / totalTyped) * 100) : 100;
...
function renderPayload() {
  ...
  for (let i = 0; i < currentPayload.length; i++) {
    let className = "";
    if (i < typed.length)
      className = typed[i] === currentPayload[i] ? "correct" : "incorrect";
...
function updateRace() {
  ...
  const playerProgress = Math.min(
    (stats.correctCharacters / currentPayload.length) * 100,
    100,
  );
  ...
  aiWpmDisplay.textContent = raceActive ? Math.round(aiCurrentWpm) : "0";
  ...
  if (stats.accuracy < 82 && stats.totalTyped > 12) {
    warning.textContent =
      "Integrity lock: fix the last typo before the payload continues.";
...
function finishRace(winner) {
  ...
  // SAVE PLAYER WPM
  playerWpmHistory.push(stats.wpm);

  // Keep only last 10 matches
  if (playerWpmHistory.length > 10) {
    playerWpmHistory.shift();
  }
  ...
  terminalStatus.textContent = playerWon
    ? "STATUS: BREACH COMPLETE"
    : "STATUS: COUNTER-BREACHED";
...
function startRace() {
  ...
  const averageWpm = getAveragePlayerWpm();

  // RANDOM OFFSET AROUND PLAYER AVERAGE
  const randomOffset = randomRange(-10, 10);

  aiBaseWpm = Math.max(12, averageWpm + currentRival.base + randomOffset);
  ...
  aiMood.textContent = "MODE: READY";
  aiWpmDisplay.textContent = "0";
  terminalStatus.textContent = "STATUS: AWAITING INPUT";
...
function resetGame() {
  ...
  clearInterval(eventInterval);
  currentPayload =
    "Click START BREACH to challenge an adaptive AI counter-intrusion engine.";
  typingInput.value = "";
  typingInput.disabled = true;
  startTime = null;
  aiProgress = 0;
  ...
  commentaryText.textContent =
    "AI: Press START BREACH. I am ready to admire your keyboard dominance.";
